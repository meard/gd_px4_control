#include "gd_px4_control/video_capture.hpp"
...
namespace gd_video_cam
{
    ...
    GDCameraNode::GDCameraNode(const rclcpp::NodeOptions &options) : Node("gd_video_cam", options),
                                                                                     canceled_(false)
    {
...
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_LOAD_PARAMETER((*this), cxt_, n, t, d)
        CXT_MACRO_INIT_PARAMETERS(GDCONTROL_ALL_PARAMS, validate_parameters)

        // Register for parameter changed. NOTE at this point nothing is done when parameters change.
#undef CXT_MACRO_MEMBER
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_PARAMETER_CHANGED(n, t)
        CXT_MACRO_REGISTER_PARAMETERS_CHANGED((*this), cxt_, GDCONTROL_ALL_PARAMS, validate_parameters)

        // Log the current parameters
#undef CXT_MACRO_MEMBER
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_LOG_SORTED_PARAMETER(cxt_, n, t, d)
        CXT_MACRO_LOG_SORTED_PARAMETERS(RCLCPP_INFO, get_logger(), "gd_video_cam Parameters", GDCONTROL_ALL_PARAMS)

        // Check that all command line parameters are registered
#undef CXT_MACRO_MEMBER
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_CHECK_CMDLINE_PARAMETER(n, t, d)
        CXT_MACRO_CHECK_CMDLINE_PARAMETERS((*this), GDCONTROL_ALL_PARAMS)
        ...
        thread_ = std::thread(std::bind(&GDCameraNode::loop, this));

        RCLCPP_INFO(get_logger(), "start publishing");
    }

    GDCameraNode::~GDCameraNode()
    {
        // Stop loop
    ...
    void GDCameraNode::validate_parameters()
    {
    }

    void GDCameraNode::loop()
    {
        cv::Mat frame;
...
RCLCPP_COMPONENTS_REGISTER_NODE(gd_video_cam::GDCameraNode)